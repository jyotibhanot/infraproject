#!/bin/bash
# Setup my infraproject environemt
pn=infraproject
...
if [ $# -lt 1 ] || [ "$1" = "help" ]; then
   ...
   echo "Commands:"
   echo
   echo "staging    Creates the staging environment"
   echo "production Creates the production environment"
...
if [ "$1" = "staging" ]; then
	if [ $# -lt 2 ]; then
        ...
        cd $1
            cmd="$2"
            if [ "$2" = "build" ]; then docker build -t staging_web ./web;fi
...
# pack
if [ "$1" = "pack" ]; then
    docker tag production_static jyotibhanot30/infraprojectstatic
    docker tag production_web jyotibhanot30/infraprojectweb
    docker push jyotibhanot30/infraprojectstatic
    docker push jyotibhanot30/infraprojectweb
	echo;exit
fi
